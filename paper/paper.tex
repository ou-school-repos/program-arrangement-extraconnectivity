...
shifting $3\!\to\!1$ fixes $13$ and breaks two adjacencies). \item \textbf{The
Algebraic Defect Framework (Section~\ref{sec:defect}):} Instead of moving
vertices geometrically, we count the coordinate-aligned lines (roots) that the
subset touches. Each root extends to exactly $(n-k+1)$ vertices, so the boundary
factors cleanly into a term proportional to $(n-k)$ minus a correction that
depends only on the subset's internal defect and cross-collisions---not on $n$
or $k$. \item \textbf{The Fixed-Topology Asymptotic Penalty
...
such subgraph is exactly $d\,2^{d-1} - d = \Eseq(R)$. \item \textbf{Case 2: $V'
\not\subseteq Q_d$.} If $V'$ contains at least one vertex $v'$ outside of $Q_d$,
then at most $R-1$ vertices of $V'$ lie within $Q_d$. By
Theorem~\ref{thm:defect}, the maximum defect among these $R-1$ vertices is
bounded by $\Eseq(R-1)$. Since $v'$ lies outside of $Q_d$, it uses at least one
fresh coordinate dimension and therefore can share at most $1$ root with $Q_d
\cap V'$. The maximum possible defect for any such spillover configuration is
thus bounded by $\Eseq(R-1) + 1$.
	\end{enumerate}
...
maximizer). \item $M(R, D)$ is non-decreasing as $D$ decreases from $\Eseq(R)$
toward $R-1$.
	\end{enumerate}
\end{conjecture}
...
constant} \\ for all $R \le 260$, giving strong numerical evidence that the
