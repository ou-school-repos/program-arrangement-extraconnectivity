...
\usepackage{accsupp} \usepackage{embedfile} \usepackage{listings} \lstset{
basicstyle=\ttfamily\footnotesize, columns=fullflexible, keepspaces=true,
breaklines=false, frame=none, xleftmargin=0.5em, aboveskip=0.8em,
belowskip=0.8em, keywordstyle=\color{blue}, commentstyle=\color{green!60!black},
stringstyle=\color{red}, } \usetikzlibrary{positioning, arrows.meta,
...
Hamming-Ball Dominance for Arrangement Graphs $A(n,k)$:}\\[0.5em] The Algebraic
Defect Framework\\[0.5em]
...
Under Propositions~\ref{prop:universal} and~\ref{prop:hb-eval}, we show that for
all $R \ge 2$ and $n \ge k \ge 1$, the minimum external boundary of any
$R$-element subset is bounded below by an explicit function of $R$, $k$, and
$(n-k)$, and that this bound is achieved by the lexicographic Hamming Ball when
$n - k \ge \lceil \log_2 R \rceil$ and $k \ge \lceil \log_2 R \rceil$. The full
...
The boundary-minimizing subset turns out to be the \emph{lexicographic Hamming
Ball}: the first $R$ binary strings $0, 1, \ldots, R-1$, embedded as vertices
via their binary expansions into the coordinate slots of $A(n,k)$. The exact
extraconnectivity (Theorem~\ref{thm:main}) depends on two constants that emerge
from building this ball vertex by vertex.
...
confirming the asymptotic limits conjectured
...
remains unchanged. The new compressed set is $C(V') = \{[4,1], [1,3]\}$.
	...
	\begin{equation*}
\extbd(C(V')) = \{[2,1], [3,1], [4,2], [4,3], [2,3], [1,2], [1,4]\} \implies
\left| \extbd(C(V')) \right| = 7
	\end{equation*}
...
theorem \cite{kruskal1963number,katona1968theorem,harper1966optimal} applied to
the arrangement graph fiber structure. Proposition~\ref{prop:hb-eval} is
computationally verified for all $R \le 260$ by constructing $\mathrm{HB}_R$ and
...
$R$-vertex subgraph via the \texttt{nauty}-based engine (Section~3); it remains
...
parameterizing our main capstone theorems with explicit hypothesis arguments
(\texttt{UniversalLowerBound} and \texttt{HBCrossCollisions}). This cleanly
isolates the classical extremal set theory from the novel algebraic machinery
...
Under Propositions~\ref{prop:universal} and~\ref{prop:hb-eval}, the Main Theorem
(Theorem~\ref{thm:main}) identifies the Hamming Ball as the dense limit. We
conjecture that the Star Graph provides the sparse limit:

\begin{conjecture}[The Sparse Limit Upper Bound]
The external boundary of any optimal connected $R$-vertex subgraph $V'$ in
...
limiting configurations collapse into a single isomorphism class because a Star
Graph on $3$ vertices is geometrically isomorphic to a path of length $2$, which
itself is the optimal Hamming Ball of size 3.
...
$V'_{\mathrm{opt}} = \mathrm{HB}_R$. Then
	...
	\end{equation}
where the $O_R(1)$ term depends only on the fixed topology (not on $n$ or $k$).
...
is a genuine constant: $X$ and $D$ are combinatorial invariants of the fixed
abstract $R$-vertex configuration and do not depend on the ambient dimensions
...
Proposition~\ref{prop:universal}). What is unconditionally true is only that the
...
\begin{table}[ht]
	\centering
\caption{Complete defect walk-through for $R=8$ in $A(n,k)$, valid for any $k
\ge 3$. Each row is verified by exhaustive search over all connected $8$-vertex
subgraphs.}
...
The numerical results confirm that at $n=100$, the Hamming Ball for $R=8$ wins
by 464 vertices against a Star Graph and by 190 vertices against the second-best
connected subgraph. In large-scale systems, this linear divergence ensures that
the Hamming Ball remains the unique boundary minimizer.

\paragraph{The Structural Inversion: Hypercubes vs.\ Arrangement Graphs.}
In recent literature concerning hypercube variants like Augmented Cubes
\cite{cheng2023augmented}, the optimal boundary-minimizing cut is frequently
constructed using the Star Graph $K_{1, R-1}$. Why does the Star Graph represent
the global optimum in hypercubes, but the absolute worst-case Sparse Limit in
arrangement graphs? Our framework provides the exact mathematical mechanism for
this \emph{structural inversion}. In hypercubes, the penalty for missing a unit
of defect is a static constant. The Star Graph produces cross-collisions that
...
with the network dimensions, it completely obliterates the static
$\binom{R-1}{2}$ savings of the Star Graph. Thus, the optimal structure
permanently inverts: in hypercubes, the minimum cut is the Sparse Limit, but in
arrangement graphs, the minimum cut is the Dense Limit.
...
\begin{table}[ht]
	\centering
\caption{Achievable defect sets $\mathcal{D}(R)$ for selected $R$. Powers of 2
($2^d$) exhibit primary fracture gaps of $d{-}1$; near-powers of 2 ($2^d-1$)
exhibit secondary fracture gaps of $d-3$ for $d \ge 4$.}
...
collapses to a single topology---the Hamming Ball---at the dense limit $D =
\Eseq(R)$. At powers of $2$, the funnel exhibits a discrete fracture: the gap
...
the final exact minimum-cut theorems with explicit hypothesis arguments
(\texttt{UniversalLowerBound} and \texttt{HBCrossCollisions}). This cleanly
isolates the classical extremal set theory from the novel algebraic machinery
while ensuring that the compiled theorems are completely transparent, verified,
and free of raw, uninstantiated axiom commands.

The capstone theorem conditionally combines a supplied universal lower bound
with the Hamming-ball construction. \emph{Note:} In the Lean source, the code
...
for all $R \le 260$, giving strong numerical evidence that the Hamming Ball
achieves the exact boundary predicted by Theorem~\ref{thm:main}. Because the
Algebraic Defect Framework decouples the network dimensions from the subset
structure, this paper leverages a property of $A(n,k)$, that the constants
$\Eseq(R)$ and $\Cconst(R)$ are strictly independent of $n$ and $k$; thus,
verifying the boundary for the dense embedding $n=2R$, $k=R$ is sufficient to
guarantee the formula's exactness for all valid dimensions.
Proposition~\ref{prop:universal} (the universal bound over \emph{all} subsets)
is verified separately by exhaustive enumeration for $R \le 10$.
