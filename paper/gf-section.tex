...
The Hamming-ball boundary comparison is governed by two integer sequences:
the cumulative binary weight $\Eseq(R)$ (OEIS A000788) and the correction
constant $\Cconst(R)$, linked by the closed form~\eqref{eq:cconst}. This
section places these coefficients in their
...
depend on Propositions~\ref{prop:restricted_lower_bound} or~\ref{prop:hb-eval}. Via
...
\begin{proof}
	...
	Thus the products $R(d{+}1)$ and the sequence $2^d$ are $2$-regular, and
	the explicit identity $\Cconst = R(d{+}1) - 2^d - \Eseq$ in
	\eqref{eq:cconst} gives the claimed regularity of $\Cconst$.
\end{proof}
...
\begin{corollary}[No linear-recurrence closed form]\label{cor:transcend}
	...
	satisfies no linear recurrence with constant coefficients, and is not
	expressible by a rational generating function or a polynomial-exponential
	closed form.
\end{corollary}
...
\begin{remark}[Relation to the Fixed-Topology Asymptotic Penalty]\label{rem:penalty-analogy}
	...
	Theorem~\ref{thm:penalty}: in the boundary comparison, dimension scaling
	contributes the exact linear term in $(n-k)$, while the correction density
