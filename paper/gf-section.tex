...
% closed forms and density decomposition for R < 2^16). This is a finite
...
% Theorem them:delange, transcendence in Corollary cor:transcend, the exact
% supremum in Remark rem:extremal), which rest on their own arguments/citations.
% ============================================================================
...
\begin{proof}
	...
	$d = \lceil \log_2 R \rceil$: the bit-length sequence $d$ is itself a
...
\begin{proof}
	...
	and $\Eseq$ is not $C$-finite, i.e.\ $F$ is not rational. The same
	conclusion holds for $\Cconst$: writing $d = \lceil \log_2 R \rceil$,
	\eqref{eq:cconst} gives $\Cconst(R) = R(d{+}1) - 2^d - \Eseq(R)$, and
	since $2^{d-1} < R \le 2^d$ we have $2^d = \Theta(R)$, so
	$R(d{+}1) - 2^d = R\log_2 R + O(R)$; combined with
	$\Eseq(R) = \tfrac{1}{2}R\log_2 R + O(R)$ this gives
	$\Cconst(R) = \tfrac{1}{2}R\log_2 R + O(R)$, the same growth class ruled
	out above, so $\Cconst$ is not $C$-finite either by the identical
	argument. A Mahler
