...
\begin{proof}
	...
	recurrence with constant coefficients, i.e.\ $\Eseq$ would be a
	$C$-finite sequence $\Eseq(R) = \sum_i p_i(R)\lambda_i^R$ for finitely
	many algebraic constants $\lambda_i$ and polynomials $p_i$; every such
	sequence grows as $\Theta(R^k\rho^R)$ for some integer $k \ge 0$ and real
	$\rho \ge 0$ (the modulus of a dominant root), up to a bounded
	oscillatory factor. But $\Eseq(R) = \tfrac{1}{2}R\log_2 R + O(R)$
	(Theorem~\ref{them:delange} below): the ratio $\Eseq(R)/R^k$ tends to
	$\infty$ for $k = 1$ and to $0$ for $k = 2$, and $\Eseq(R)/R^k$ cannot
	tend to a nonzero constant for any $\rho \ne 1$ since $\Eseq(R)$ is
	subexponential; so no integer $k$ and real $\rho$ match this growth,
	and $\Eseq$ is not $C$-finite, i.e.\ $F$ is not rational. A Mahler
	function that is algebraic over $\mathbb{Q}(x)$ is rational by the
	theorem of Adamczewski and Bell~\cite{adamczewski2017problem}; since $F$
	satisfies~\eqref{eq:mahler} and is irrational, it is transcendental.
\end{proof}
