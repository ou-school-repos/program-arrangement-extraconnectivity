...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.8\textwidth]{../assets/out/complexity-curves.pdf}
...
each a localized geometric dead-end of $A(10,5)$.  This floor is an unverified
search result from a single-solve diagnostic, not a certified infeasibility
certificate for boundaries below $\CertFloor{}$.  The certified containment is
therefore
\[
...
\begin{remark}[Coordinate-boundary incidence counting]\label{rem:coord-edges}
Let $\CoordEdges(S)$ denote the total number of distinct coordinate-boundary
incidences, counted separately for each coordinate direction:
...
distinct boundary vertices.  The second equality counts, per occupied
coordinate-root $r$, the $(m+1)$ vertices on its line minus those already
selected into $S$; every such external vertex is adjacent to all members on
that line.
