...
This section records a fixed-configuration comparison derived from the exact
boundary identity. It is not an exact profile theorem: the proposed
Hamming-optimality inequality is false (Proposition~\ref{prop:restricted_lower_bound}),
and the unconditional quantitative statement is the Hamming sandwich in
Section~\ref{sec:hamming-sandwich}. The following proposed sparse-limit
comparison is also refuted:
...
\begin{figure}[H]
	...
	the Hamming-cube configuration $Q_d$ ($D = \Eseq(R)$).}
	\label{fig:limits}
\end{figure}
...
\begin{theorem}[Fixed-configuration asymptotic penalty]\label{thm:penalty}
...
Consequently, for a fixed realizable configuration family whose defect and
collision data remain unchanged as the alphabet is enlarged, there is a
threshold in $n-k$ beyond which that family has larger boundary than the
Hamming configuration. This is a pointwise comparison of fixed patterns, not
a uniform statement over patterns that themselves vary with $(n,k)$.
\end{theorem}
...
topologies (indeed, a stronger candidate bound is refuted by an
