...
\begin{definition}[Full Star spine]\label{def:star}
Fix a center $c$ and let $J$ be a set of $j$ active coordinates.  For each
$p\in J$, replace $c_p$ by each of the $m=n-k$ symbols unused by $c$,
leaving all other coordinates unchanged.  The resulting full Star is
\[
 S_j(c)=\{c\}\cup\{c[p\leftarrow s]:p\in J, s\notin c\},
 \qquad |S_j(c)|=1+jm.
\]
Its closed-form defect and external multiplicity excess are
\[
D(S_j)=jm,\qquad X(S_j)=m(m-1)\binom{j}{2}.
\]
The ``canonical Star'' used in finite traces is an ordering-dependent prefix
of this spine; it is not a definition of the full Star for arbitrary volume.
\end{definition}
...
On the Boolean-cube embedding range, the first $R$ binary strings
$0,1,\ldots,R-1$, embedded into the coordinate slots of $A(n,k)$, form the
\emph{lexicographic Hamming Ball}. Its boundary is exactly evaluated in
Proposition~\ref{prop:hb-eval}; it need not be the boundary minimum. The
unconditional comparison with the minimum profile is the additive-error
Hamming sandwich (Theorem~\ref{thm:hamming-sandwich}). Its value depends on
two constants that arise from building the ball vertex by vertex.
...
subset (Theorem~\ref{thm:defect}), and $\Cconst(R)$ is the correction in the
boundary of the Hamming-ball candidate (Proposition~\ref{prop:hb-eval}), satisfying
