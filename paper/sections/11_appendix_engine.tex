...
engine checks the candidate evaluation in Proposition~\ref{prop:hb-eval} by verifying \\
\texttt{brute\_force\_neighbors(verts, 2 * R, R) == (R * R - nk1) * R -
constant} \\ for all $R \le 160$, checking the Hamming Ball's boundary
formula. This does not establish that the Hamming Ball minimizes boundary.
...
thus, verifying the boundary for the dense embedding $n=2R$, $k=R$ checks the
construction's formula, not its optimality in every valid dimension.
Proposition~\ref{prop:restricted_lower_bound} quantifies over \emph{all} subsets, whereas
the \texttt{nauty}-based search tests connected $R$-vertex subsets for
$R \le 10$; a separate exhaustive checker (\texttt{universal\_check},
\texttt{src/universal\_lower\_bound.cpp}) tests \emph{all} subsets, including
disconnected ones, for $R \le 6$ across 30 parameter rows of $A(n,k)$. Both
are therefore partial evidence rather than a verification of the universal
proposition for general $R$.
