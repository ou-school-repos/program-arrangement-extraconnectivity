...
The former exact-minimum claim $\Phi_{n,k}(R)=H(R,n,k)$ is withdrawn.
The embedding conditions ensure that the Hamming-ball candidate exists;
they do not make it optimal. The full-Star certificate in
...
$\Phi_{n,k}(R)\leq H$. Applying \eqref{eq:hamming-linear-lower} to a
minimizer and using $H\leq(Rk-E(R))m$ gives the other side of
\eqref{eq:hamming-profile-sandwich}. The Lean development is in
\texttt{Arrangement/ArrangementExtraconnectivity.lean},
\texttt{Arrangement/CrossTop.lean}, and
\texttt{Arrangement/Capstone.lean}; the public theorem is
\texttt{Arrangement.arrangement\_boundary\_sandwich}.
\end{proof}
