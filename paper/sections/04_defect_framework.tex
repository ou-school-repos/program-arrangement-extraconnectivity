...
\begin{conjecture}[Restricted Boundary Inequality]\label{prop:restricted_lower_bound}
...
The unrestricted form of this inequality (without the embedding conditions)
fails: in $A(10,8)$, the full Star with $R=17$ has boundary $168$, below
the target $169$. More importantly, the embedding conditions do not by
themselves establish the claim for general $m$. For example, the full Star in
$A(12,6)$ at $R=37$ satisfies
$k=m=6\ge\lceil\log_2 37\rceil$, but has boundary slack $-57$ relative to
the proposed lower bound. The conditions are therefore a useful motivation
for the Hamming embedding, not a proved safe-range criterion. Computational
verification confirms the restricted bound for all tested parameters with
$m\le4$; the general geometric statement remains open and must be given an
additional range or shape hypothesis.
\end{remark}
...
The remaining step to resolve Proposition~\ref{prop:restricted_lower_bound} via this route is a coverage-avoidance inequality bounding $L_c$. This inequality remains open; the finite CP-SAT searches reported in Appendix~\ref{app:computational} are diagnostic evidence only.
...
\texttt{RestrictedLowerBound} hypothesis, active under the hypercube embedding conditions.
...
\textup{(Conditional on Conjecture~\ref{prop:restricted_lower_bound}
and~\ref{prop:hb-eval}.)} Assume the hypercube embedding conditions
