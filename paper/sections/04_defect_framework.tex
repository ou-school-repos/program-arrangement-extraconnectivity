...
\begin{proposition}[Restricted Boundary Inequality]\label{prop:restricted_lower_bound}
...
the target $169$. However, the embedding conditions $R \le 2^m$ restrict
$R$ to a range that lies entirely below the failure threshold for $m \le 4$.
Specifically, $2^m < 1 + j_{\mathrm{fail}} \cdot m$ for $m \le 4$, where
$j_{\mathrm{fail}}$ is the cumulative-slack failure point of the full
Star. Computational verification confirms the restricted bound for all
parameters with $m \le 4$; the proof for general $m$ remains open.
\end{remark}
...
\textup{(Conditional on Propositions~\ref{prop:restricted_lower_bound}
