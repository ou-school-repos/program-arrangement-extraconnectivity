...
\begin{proposition}[Former Universal Boundary Inequality (refuted)]\label{prop:universal}
	\textbf{The following unrestricted assertion is false; it is retained
	only to identify the hypothesis of the conditional results below.}
...
\begin{remark}[Full-Star counterexample]
In $A(10,8)$, take the center $(0,1,2,3,4,5,6,7)$ and all sixteen
vertices obtained by replacing one coordinate with 8 or 9.
This set has $R=17$, $D=16$, and $X=56$.
Since $\Eseq(17)=33$ and $\Cconst(17)=37$, its external boundary
has size $168$, whereas \eqref{eq:universal-bound} demands $169$.
The proposed restriction $n-k\ge\lceil\log_2 R\rceil$ excludes this
witness, but its sufficiency for a revised lower bound remains unproved.
The counterexample refutes the numerical target without asserting that
a Hamming ball achieving that target embeds in this parameter cell.
\end{remark}
...
$\delta_D=\Eseq(R)-D(V')$. Then Proposition~\ref{prop:universal} for $V'$
is equivalent to
...
right-hand side of \eqref{eq:universal-bound}, and rearrange.
\end{proof}
...
The remaining step to resolve Proposition~\ref{prop:universal} via this route is a coverage-avoidance inequality bounding $L_c$. This inequality remains open; the finite CP-SAT searches reported in Appendix~\ref{app:computational} are diagnostic evidence only.
...
refuted for general $R$ by the full-Star counterexample above. Conceptually,
...
Proposition~\ref{prop:universal} above. Why the exact statement resists the
