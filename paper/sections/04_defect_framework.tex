...
\paragraph{Coordinate-local incidence route (unverified dictionary).}
Fix a coordinate $c$ and project the symbol slices $S_i$ to local sets
$Q_i\subseteq V(A(n-1,k-1))$.  For $x\in\pi_c(S)$ with $i\notin x$, let
$x^{(i)}$ be the global vertex obtained by reinserting $i$ at coordinate $c$.
The sweep argument must use an explicit incidence set rather than identify a
global boundary vertex with a local-boundary vertex implicitly; one natural
candidate is
\[
 \mathcal I_c=\{(i,x):x\in\pi_c(S),\ i\notin x,\
 x\in\partial_{\mathrm{loc}}Q_i\}.
\]
The proposed relation between $X(S)-\sum_iX(Q_i)$ and
$|\mathcal I_c|$ has not yet been verified with consistent local/global
boundary conventions, and is not used as a lemma here.  Establishing the
appropriate incidence identity, followed by a coverage-avoidance inequality,
remains open.  The finite CP-SAT searches reported in
Appendix~\ref{app:computational} are diagnostic evidence only.
