...
Superseded: the active development proves `cross_collision_bound_factor_one`
using an excess-multiplicity estimate and the ordered source-pair charging
bound. The alternative unordered-overlap route below remains archived and is
not used by the active proofs.
...
/-- The factor-one estimate, stated here only as the archived interface. -/
def CrossCollisionBound {n k : ℕ} (V' : Finset (ArrVertex n k)) : Prop :=
  cross_collisions V' ≤ V'.card * (V'.card - 1)

/-- The unordered Bonferroni estimate proposed as an alternative proof route. -/
...
/-- The unordered coordinate-pair charging estimate proposed as an alternative
    second ingredient. -/
