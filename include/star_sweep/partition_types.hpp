...
struct PartitionSpec {
    ...
    std::pair<std::size_t, std::size_t> range(std::size_t total_items) const {
        validate();
        const std::size_t items_per_partition = total_items / count;
        const std::size_t extra_items = total_items % count;
        const std::size_t first =
            items_per_partition * index + std::min(index, extra_items);
        const std::size_t last = first + items_per_partition +
                                 static_cast<std::size_t>(index < extra_items);
        return {first, last};
    }
};
