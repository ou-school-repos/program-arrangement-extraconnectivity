...
- **`UniversalLowerBound` is refuted as an unrestricted statement, and has no
  ...
  finite witnesses, and the additive convex fiber-size Lyapunov ansatz Ψ*f(V') =
  ...
  to be formalized, but any such approach must now target a \_restricted* form
  of the inequality, not the unrestricted one, since the latter is false.
