...
- **Formal Verification (Lean 4):** The stable Lean build machine-verifies the algebraic defect framework, core arithmetic, explicit Hamming-ball construction, and its collision evaluation. The remaining extremal-combinatorics dependency is `RestrictedLowerBound` (gated by hypercube embedding conditions).
