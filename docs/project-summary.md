...
- **Edge Isoperimetry & Harper's Theorem:** Applying Harper's Edge Isoperimetric Theorem to the arrangement graph, we identify that the optimal vertex subsets maximizing internal edges (and thus minimizing the external boundary) are Hamming balls (initial segments of the binary-reflected Gray code).
...
- **Neighbor-Set Closed-Form Expression:** The exact extraconnectivity function $\kappa_R$ is evaluated as:
  ...
  where $C(R) = (R-1) + \sum*{x=1}^{R-1} L(x) - \text{A000788}(R)$ accounts for internal symbol collisions. Although the exact correction terms differ from the 2022 paper, both models converge to the same asymptotic equivalence of $(R)k(n-k)$ as $n,k \to \infty$.
...
The project bridges the gap between empirical computational search and theoretical graph properties. By capturing the isoperimetric behavior of arrangement graphs and identifying the structural breakdown at $R=8$, we replaced an overfitted linear model with a robust, hypercube-based theory. The resulting $O(R^4)$ predictor, paired with exact formal Lean 4 verification, provides the definitive, scalable blueprint for computing the fault tolerance of arrangement graphs.
