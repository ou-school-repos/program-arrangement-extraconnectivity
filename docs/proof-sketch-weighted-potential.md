...
exhaustive, random, structured, Swiss, and crumbs corpora) supports the
inequality with no counterexample found. The proof target is precise: show
X(V') + (m+1)D(V') ≤ C(R) + m·E(R) for all V', by any of the strategies above or
another route. The existing Cheng et al. computational results
