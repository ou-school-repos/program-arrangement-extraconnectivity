...
$$
\Phi_{n,k}(R)=\min\{|N(S)|:S\subseteq V(A(n,k)),\ |S|=R\},
$$
...
| Graph    | $m=n-k$ | $R$ values | $\Phi_{n,k}(R)$, in order       | Hamming-ball witness available and sharp |
| -------- | ------: | ---------: | ------------------------------- | ---------------------------------------- |
...
The radius-one Star has volume $R_*=1+k(n-k)$ and boundary

$$
|N(B_1(v))|=\binom{k}{2}(n-k)(n-k+1).
$$

| Graph    | Star volume $R_*$ | Computed profile | Star boundary | Observation                                                      |
| -------- | ----------------: | ---------------: | ------------: | ---------------------------------------------------------------- |
...
boundaries are disjoint and avoid the other components, hence

$$
|N(S)|=\sum_i |N(C_i)|.
$$

Let $\psi_{n,k}(r)$ be the minimum boundary among $r$-sets connected in this
distance-two graph. For a disconnected $S$ with component sizes
$r_1,\ldots,r_t$,

$$
|N(S)|\geq \sum_i \psi_{n,k}(r_i).
$$

Thus a disconnected-split calculation gives a lower bound on the unrestricted
profile. It is not automatically an equality: independently minimizing
components need not be placeable at mutual distance at least three. To certify
that a distance-two-connected minimizer is also unrestricted, it suffices to
check that every nontrivial integer split has lower-bound sum greater than or
equal to that minimizer's boundary.
...
distance-two-connected minimum is 27, this certifies $\Phi_{6,4}(6)=27$ against
disconnected sets as well.
