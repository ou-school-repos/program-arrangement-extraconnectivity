...
// Accumulate in 128 bits.  Each fixed-set count is <= C(N,R), so the sum is
// <= |G| * C(N,R); main() refuses inputs where that bound could exceed 2^126.
...
std::vector<Count> burnside(int n, int k, int maximum) {
    ...
    for (int r = 0; r <= maximum; ++r) {
        if (totals[r] % group_size != 0) {
            std::cerr << "internal error: Burnside sum not divisible at R=" << r
                      << '\n';
            std::exit(3);
        }
        totals[r] /= group_size;
