...
def analyze(A, S, c=0):
    ...
    gradx = {
        x: sum(len(Sx.get(y, set()) - Sx[x]) for y in locN[x] if y in pi)
        for x in pi
    }
    ...
    U = sum(
        len({tuple(v[p] for p in range(k) if p != q) for v in S})
        for q in range(k)
    )
    D = k * R - U
    Dloc = {}
    for i in range(n):
        ...
        for q in range(k - 1):
            proj = {
                tuple(x[p] for p in range(k - 1) if p != q) for x in Q[i]
            }
            u += len(proj)
    ...
    multi_ok = dE >= conc
    rec = dict(
        R=R, pi=len(pi), iff=iff_ok, perx=perx_ok, Lc=Lc, grad=grad,
        recomb=recomb, conc=conc, dC=dC, dE=dE, T=T, gap=gap,
        target=target_ok, multi=multi_ok,
        suff_fail=(grad + (m + 1) * conc > dC + m * dE),
    )
    rows = [
        dict(n=n, k=k, m=m, R=R, pi=len(pi), i=i, Ri=len(Q[i]),
             Pi=len(P[i]), C=Cconst(len(Q[i])), E=Eseq(len(Q[i])),
             Dloc=Dloc[i], Lci=len(Lc_i[i]))
        for i in range(n)
    ]
    return rec, rows
...
def corpus():
    ...
    specs = [
        (A43, 4, 150), (A43, 6, 150), (A43, 8, 60),
        (A53, 5, 150), (A53, 8, 150), (A53, 10, 80),
        (A63, 6, 120), (A63, 9, 100), (A63, 12, 60),
    ]
...
def main():
    ...
    for tag, A, S in corpus():
        ...
        srows.extend(rows)
        ok = (
            r["iff"] and r["perx"] and r["recomb"] == r["conc"]
            and r["target"] and r["multi"]
        )
        if not ok:
            print(f"FAIL {tag}: {r}")
            fails += 1
    ...
    with open(csv_path, "w", newline="") as f:
        w = csv.DictWriter(f, fieldnames=["n", "k", "m", "R", "pi", "i",
                                          "Ri", "Pi", "C", "E", "Dloc", "Lci"])
        w.writeheader()
        w.writerows(live)
