...
print("B3 OGF sum_bit_length: F_L=(x/(1-x)^2)*sum x^{2^j}:", ok)
...
print("C1 Mahler: S(x)=(1+x)S(x^2)+x/(1-x^2):", ok)
...
print("C2 Mahler for F_E: x*F_E(x)=(1+x)^2 F_E(x^2)+x^3/((1-x)(1-x^2)):", ok)
...
for R in range(2, 1 << 16):
    ...
    if abs(C(R) / R - (0.5 * lg + Psi - Phi)) > 1e-9:
        ...
        print("FAIL", R)
        break
print("C(R)/R = (1/2)log2R + Psi({log2R}) - Phi(log2R):", ok)
# X_HB/R = Psi - 2Phi bounded, zero at powers of two, nonneg
X = [((C(R) - Ev[R]) / R) for R in range(2, 1 << 16)]
print(
    "X/R range: [%.4f, %.4f]; X(2^d)=0 check:" % (min(X), max(X)),
