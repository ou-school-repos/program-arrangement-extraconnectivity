...
for R in Rs:
    ...
    Xv.append((Cr - Ev[R]) / R)
ax = axes[1]
ax.plot(Rs, Xv, "-", lw=0.6, color="#1a4a7a", label=r"$X_{\mathrm{HB}}(R)/R$")
ax.plot(
    Rs,
    Cv,
    "-",
...
ax.text(3, 0.515, r"$\sup = 1/2$ (alternating-binary $R$)", fontsize=9.5, color="gray")
for d in range(2, 16):
